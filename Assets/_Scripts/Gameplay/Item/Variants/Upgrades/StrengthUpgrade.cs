...
public class StrengthUpgrade : Item
{
    protected override void ExecuteItemAction()
    {
        if (player == null)
        {
            Debug.LogWarning("Player reference is not set. Cannot apply strength upgrade.");
            return;
        }
        ...
        playerUpgrades.ApplyUpgrade(UpgradeType.Strength);
    }
}
